...
if (function_exists('spl_autoload_register')) {
	...
	if ( ! DOMPDF_AUTOLOAD_PREPEND OR $funcs === FALSE) {
	...
	elseif (PHP_VERSION_ID >= 50300) {
		...
		foreach ($funcs as $func) { 
			if (is_array($func)) {
				// TRICKY: There are some compatibility issues and some places where we need to error out
				$reflector = new ReflectionMethod($func[0], $func[1]);
				if ( ! $reflector->isStatic()) {
					throw new Exception('This function is not compatible with non-static object methods due to PHP Bug #44144.');
				}

				// Suprisingly, spl_autoload_register supports the Class::staticMethod callback format, although call_user_func doesn't
